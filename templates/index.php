      ...
      <div id="mdnj-tab-1" class="mdnj-tab-content <?php echo $is_settings_page ? 'current' : ''; ?>">
        <form method="post" action="">
          ...
          if (mdnj_is_api_key_authorized() && !$is_plain_permalinks) {
            ...
            echo '<a href="' . esc_url(mydataninja_config('FRONT_BASE_URL')) . '/dashboard" class="mdnj-btn mdnj-authorize-btn">Open MyDataNinja</a>';
          } else {
            if ($is_plain_permalinks) {
              ...
              echo '<a href="' . esc_url(mydataninja_config('FRONT_BASE_URL')) . '/crm/woocommerce?name=' . esc_html(get_bloginfo('name')) . '&currency=' . esc_html(get_woocommerce_currency()) . '&base_url=' . esc_url(home_url()) . '" class="mdnj-btn mdnj-authorize-btn">Authorize</a>';
            }
          }
          ?>

...
{
  global $woocommerce, $post, $mdnj_cog_field_name;
...
function mdnj_add_cost_of_goods_field_to_variations($loop, $variation_data, $variation)
{
  ...
  woocommerce_wp_text_input([
    'id' => $mdnj_cog_field_name . '[' . $variation->ID . ']',
    'label' => __('Cost of Goods', 'mydataninja-ad-performance-tracking-order-reports-crm-analytics-and-optimization-tools') . ' (' . get_woocommerce_currency_symbol() . ')',
    'desc_tip' => 'true',
    'description' => __('Enter the cost of goods for calculating profit.', 'mydataninja-ad-performance-tracking-order-reports-crm-analytics-and-optimization-tools'),
    'value' => get_post_meta($variation->ID, $mdnj_cog_field_name, true),
...
function mdnj_save_cost_of_goods_field($post_id)
{
  ...
  $cost_of_goods = isset($_POST[$mdnj_cog_field_name]) ? sanitize_text_field($_POST[$mdnj_cog_field_name]) : '';

  update_post_meta($post_id, $mdnj_cog_field_name, $cost_of_goods);
}
...
function mdnj_save_cost_of_goods_field_for_variations($variation_id, $i): void
{
  ...
  $cost_of_goods = isset($_POST[$mdnj_cog_field_name][$variation_id]) ? sanitize_text_field(wp_unslash($_POST[$mdnj_cog_field_name][$variation_id])) : '';
  if (!empty($cost_of_goods)) {
    update_post_meta($variation_id, $mdnj_cog_field_name, $cost_of_goods);
  }
}
...
{
  global $post, $mdnj_cog_field_name;

  $cost_of_goods = get_post_meta($post->ID, $mdnj_cog_field_name, true);
